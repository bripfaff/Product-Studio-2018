...
df = pd.read_csv('ibov_full.csv', sep=",", encoding = "ISO-8859-1")
actual= pd.read_csv("ibov_returns.csv", sep=",")
...
def make_prediction(b_matrix, word_list, tweets):
    ...
    for sentence in tweets["Tweet"]:
        ...
        p3=1
        p4=1
        for w12 in sentence.split():
            if w12 in b_matrix.columns:
                s = np.sum(b_matrix.loc[:,w12])
                p1 = p1 * np.log(b_matrix.iloc[0][w12]/s)
                p2 = p2 * np.log(b_matrix.iloc[1][w12]/s)
                p3 = p3 * np.log(b_matrix.iloc[2][w12]/s)
                p4 = p4 * np.log(b_matrix.iloc[3][w12]/s)
        tweet_pred.append(np.argmax([p1,p2,p3,p4]))
    return(stats.mode(tweet_pred).mode) 

# def update_wl(word_list, tweets): 
...
def stem(df):
    ...
    for i in range(len(df)):
        sentence2=str(df["Tweet"].iat[i]).replace("/","").replace("@","").replace("*","").replace("#","")\
        ...
        for word in sentence2.split():
            if word in stop_words or 'http' in word or 'pic.twitter' in word:
                s = s
            else:
                s += word+ " "
        df["Tweet"].iat[i] = s 
...
#             word_list = update_wl(word_list, tweets)
#             b_matrix = update_b_matrix(b_matrix, word_list, tweets)
            preds.append(pred)
            ...
            dpred.to_csv("new_predictions_v2_2018.csv", mode='a' , index= False, header=False)
        start_date+= dt.timedelta(days=periods)
    return(preds, dat, rets,  b_matrix)
    
   
start_date= pd.to_datetime("12/30/2015")
end_date= pd.to_datetime("9/1/2018")
period=7
df=date_return_match(start_date, end_date, df, actual, period)

start_date= pd.to_datetime("5/7/2017")
end_date= pd.to_datetime("5/30/201")
period=7
